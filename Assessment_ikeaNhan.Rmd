...
ikea.rf <- randomForest(rating_discount ~. , data = ikea, mtry = 3,
...
```
```{r}
index=sample(2,nrow(ikea),replace = TRUE,prob = c(0.7,0.3))
```
```{r}
Training= ikea[index==1,]
Testing= ikea[index==2,]
```
```{r}
RFM= randomForest(discount~.,data = Training)
```
```{r}
discount_pred= predict(RFM,Testing)
Testing$discount_pred=discount_pred
View(Testing)
```
```{r}
CFM=table(Testing$discount,Testing$discount_pred)
CFM
